  ...
  updateSummary = (selectedItems) =>{
    ...
    Object.keys(selectedItems).map(myKey=>
      <div className="summary__option" key= {myKey}>
        ...
        <div className="summary__option__value"> {selectedItems[myKey].name} </div>
        <div className="summary__option__cost">
          {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD'})
                      .format(selectedItems[myKey].cost) }
        </div>
      </div>)
  }
    ...
    Object.keys(features)
          .map(myKey => {
            const options = features[myKey].map((item, index) => {
              const selectedClass = item.name === itemsSelected[myKey].name ? 'feature__selected' : '';
              const featureClass = 'feature__option ' + selectedClass;
              return <li key= {index} className="feature__item">
                <div className={featureClass}
                  
                  onClick={e => this.props.handleUpdateSummary(myKey, item)}>
                    { item.name }
                    ({ new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD'})
                      .format(item.cost) })
                </div>
            ...
            return <div className="feature" key={myKey}>
              <div className="feature__name">{myKey}</div>
